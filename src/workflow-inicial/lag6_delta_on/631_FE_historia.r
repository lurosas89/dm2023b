...
PARAM <- list()
PARAM$experimento <- "FE6310-lag6-delta-off"

PARAM$exp_input <- "DR6210-lag6-delta-off"
...
if (PARAM$lag6) {
  ...
  dataset[, paste0(cols_lagueables, "lag6") := shift(.SD, 3, NA, "lag"),
    by = numero_de_cliente,
    .SDcols = cols_lagueables
  ]
  ...
  for (vcol in cols_lagueables)
  {
    dataset[, paste0(vcol, "delta6") := get(vcol) - get(paste0(vcol, "lag6"))]
  }
